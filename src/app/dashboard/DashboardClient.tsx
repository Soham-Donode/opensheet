...
import { 
  BookOpen, BrainCog, FolderHeart, Sparkles, Merge, 
  MoreVertical, Pin, Pencil, Trash, FileText, ArrowRight
} from "lucide-react";
...
const popularSheets = [
  ...
  },
  {
    ...
    href: "/sheet/blind-75",
    icon: <FolderHeart className="w-8 h-8 text-[#88AB8E] dark:text-[#88AB8E]" />,
  },
];

export default function DashboardClient({ customSheets }: { customSheets: any[] }) {
  const { user } = useUser();
    ...
    <div className="w-full min-h-full flex flex-col p-4 md:p-8 max-w-7xl mx-auto space-y-12">
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-stretch w-full">
        ...
        <div className="w-full lg:w-80 shrink-0 bg-[#e9efea] dark:bg-neutral-900 border border-neutral-200 dark:border-white/10 rounded-[2rem] p-6 sm:p-8 flex flex-col justify-center gap-4 shadow-2xl backdrop-blur-3xl relative overflow-hidden">
          ...
          <h2 className="text-lg font-bold font-sans text-neutral-900 dark:text-white relative z-10">Quick Actions</h2>
      ...
      <section className="flex flex-col gap-6">
        <div className="flex items-center gap-2">
          <h2 className="text-2xl font-sans font-bold text-neutral-900 dark:text-white">Popular Curated Sheets</h2>
          <span className="bg-[#88AB8E]/10 text-[#88AB8E] px-2.5 py-0.5 rounded-lg text-xs font-bold uppercase tracking-wider">{popularSheets.length}</span>
        </div>
      ...
      <section className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h2 className="text-2xl font-sans font-bold text-neutral-900 dark:text-white">Your Custom Sheets</h2>
            <span className="bg-neutral-200 dark:bg-white/10 text-neutral-600 dark:text-neutral-300 px-2.5 py-0.5 rounded-lg text-xs font-bold uppercase tracking-wider">{customSheets.length}</span>
          </div>
        </div>
        ...
        {customSheets.length === 0 ? (
          <div className="w-full bg-white/50 dark:bg-neutral-900/50 border border-dashed border-neutral-300 dark:border-white/10 rounded-[2rem] p-12 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-bold text-neutral-900 dark:text-white mb-2">No custom sheets yet</h3>
            <p className="text-neutral-500 max-w-sm mb-6">Create a dynamic list of tailored problems or merge existing sheets.</p>
            <Button
              onClick={() => router.push('?create=true')}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {customSheets.map((sheet) => (
              <div key={sheet.id} className="group relative bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-white/10 rounded-2xl p-5 hover:border-[#88AB8E]/50 transition-all hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:hover:shadow-none hover:-translate-y-1">
                <Link href={`/sheet/${sheet.slug}`} className="block h-full cursor-pointer absolute inset-0 z-0" />
                <div className="relative z-10 flex items-start justify-between min-h-16">
                  <div className="flex items-center gap-3">
                    <div className={cn(
                      "w-10 h-10 rounded-xl flex items-center justify-center",
                      sheet.isPinned ? "bg-[#88AB8E]/10 text-[#88AB8E]" : "bg-neutral-100 dark:bg-white/5 text-neutral-500"
                    )}>
                      {sheet.isPinned ? <Pin className="w-5 h-5" /> : <FileText className="w-5 h-5" />}
                    </div>
                    <div>
                      <h3 className="font-bold text-neutral-900 dark:text-white truncate max-w-[120px]" title={sheet.name}>
                        {sheet.name}
                      </h3>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg hover:bg-neutral-200 dark:hover:bg-neutral-800 text-neutral-400">
                        <MoreVertical className="w-4 h-4" />
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="w-44 p-1.5 shadow-xl bg-white dark:bg-neutral-900 rounded-[14px] border-neutral-200/50 dark:border-white/10">
                      <DropdownMenuItem 
                        ...
                        onClick={() => startTransition(() => togglePinUserSheet(sheet.id, !sheet.isPinned))}
                      >
                        <Pin className="w-4 h-4" /> {sheet.isPinned ? "Unpin Sheet" : "Pin Sheet"}
                      </DropdownMenuItem>
                      <DropdownMenuItem 
                        className="cursor-pointer gap-2 py-2 px-3 rounded-xl font-medium focus:bg-[#88AB8E]/10 focus:text-[#4A644F] dark:focus:bg-[#88AB8E]/20 dark:focus:text-[#E2EBE4]"
                        onClick={() => {
